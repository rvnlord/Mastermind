...
namespace SeriousMastermind
{
    ...
    public class MainActivity : Activity
    {
        ...
        protected override void OnCreate(Bundle bundle)
        {
            ...
            _btnShowStory = FindViewById<Button>(Resource.Id.btnShowStory);
            ...
            _btnShowStory.Click += btnShowStory_Click;
        ...
        private async void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            if (GameState == GameState.Won)
            {
                ...
                await Task.Run(() =>
                {
                    try
                    {
                        Statistics.AddResult(new Statistic
                        {
                            Name = _txtName.Text,
                            CodeLength = Convert.ToInt32(_txtCodeLength.Text),
                            Tries = Convert.ToInt32(_txtTriesNumber.Text) - Tries - 1
                        });
                        var stats = Statistics.GetTop(10)?.ToList();
                        RunOnUiThread(() => _lvStatistics.Adapter = new StatisticsAdapter(stats));
                    }
                    catch (Exception ex)
                    {
        ...
        private void btnNewGame_Click(object sender, EventArgs e)
        {
            ...
            //_code = Enumerable.Range(0, CodeLength).Select(v => (ColorsAvailable)Rng.Next(1, 8)).ToArray(); // z powtórzeniami
            _code = Enumerable.Range(1, 8).Select(v => (ColorsAvailable)v).RandomSubset(CodeLength, Rng).ToArray(); // bez powtórzeń
        ...
        private void btnShowStory_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(_txtName.Text) || _txtName.Text.Length < 3 || new ServiceDbContext().Statistics.Select(x => x.Name).Distinct().All(p => !p.Eq(_txtName.Text)))
                return;

            var intent = new Intent(this, typeof(StoryActivity));
            intent.PutExtra("PlayerName", _txtName.Text);
